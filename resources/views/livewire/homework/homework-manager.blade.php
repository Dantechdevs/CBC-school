<div>
                ...
                <tbody class="bg-white divide-y divide-gray-100">
                    ...
                    <tr class="hover:bg-gray-50">
                        <td class="px-4 py-3">
                            ...
                            <p class="text-xs text-gray-400">by {{ $hw->teacher->full_name ?? 'Unknown' }}</p>
                        </td>
                        <td class="px-4 py-3 text-sm text-gray-700">{{ $hw->schoolClass->name ?? '—' }}</td>
                        <td class="px-4 py-3 text-sm text-gray-700">{{ $hw->learningArea->name ?? '—' }}</td>
                        <td class="px-4 py-3 text-sm">
                            <span class="{{ $hw->is_overdue ? 'text-red-600 font-medium' : 'text-gray-700' }}">
                                {{ $hw->due_at->format('d M Y, H:i') }}
                            </span>
                        </td>
                        <td class="px-4 py-3 text-sm text-gray-700">{{ $hw->submissions_count }}</td>
                        <td class="px-4 py-3">
                            <button wire:click="openGrading({{ $hw->id }})" class="text-blue-600 hover:text-blue-800 text-xs font-medium">
                                View / Grade
                            </button>
                        </td>
                    </tr>
            ...
            <div class="bg-white rounded-xl shadow-xl w-full max-w-lg p-6 max-h-[90vh] overflow-y-auto">
                ...
                <div class="space-y-3">
                    <div>
                        ...
                        @error('classId') <span class="text-xs text-red-600">{{ $message }}</span> @enderror
                    </div>
                    <div>
                        ...
                        @error('learningAreaId') <span class="text-xs text-red-600">{{ $message }}</span> @enderror
                    </div>
    ...
    @elseif($view === 'grade' && $gradingHomework)
        <div class="flex items-center gap-3 mb-6">
            <button wire:click="backToList" class="text-gray-500 hover:text-gray-700">&larr; Back</button>
            <div>
                <h2 class="text-xl font-bold text-gray-800">{{ $gradingHomework->title }}</h2>
                <p class="text-xs text-gray-400">{{ $gradingHomework->schoolClass->name ?? '' }} — due {{ $gradingHomework->due_at->format('d M Y, H:i') }}</p>
            </div>
        </div>
        ...
        <div class="bg-white rounded-xl shadow-sm overflow-hidden">
            <table class="min-w-full divide-y divide-gray-200">
                ...
                <tbody class="bg-white divide-y divide-gray-100">
                    @foreach($gradingLearners as $learner)
                    <tr>
                        <td class="px-4 py-3 text-sm font-medium text-gray-800">{{ $learner->full_name }}</td>
                        <td class="px-4 py-3">
                            <input wire:model="grades.{{ $learner->id }}.score" type="number" min="0" max="100"
                                   class="w-20 border border-gray-300 rounded-lg px-2 py-1 text-sm">
                        </td>
                        <td class="px-4 py-3">
                            <select wire:model="grades.{{ $learner->id }}.rubric_level" class="border border-gray-300 rounded-lg px-2 py-1 text-sm">
                                <option value="">—</option>
                        ...
                        <td class="px-4 py-3">
                            <input wire:model="grades.{{ $learner->id }}.feedback" type="text"
                                   class="w-full border border-gray-300 rounded-lg px-2 py-1 text-sm" placeholder="Optional remark">
                        </td>
                        <td class="px-4 py-3">
                            <span class="px-2 py-0.5 rounded-full text-xs font-medium
                                {{ ($grades[$learner->id]['status'] ?? 'pending') === 'graded' ? 'bg-green-100 text-green-700' : 'bg-gray-100 text-gray-500' }}">
                                {{ ucfirst($grades[$learner->id]['status'] ?? 'pending') }}
                            </span>
                        </td>
                    </tr>
                    @endforeach
